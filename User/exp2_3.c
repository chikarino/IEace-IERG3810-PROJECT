...
// RM0008 — RCC->CR, RCC->CFGR, FLASH->ACR
void clocktree_init(void)
{
  u8 PLL  = 7;                 // PLLMUL ：0b0111 "×9"（ CFGR PLLMUL[21:18] ）
  unsigned char temp = 0;      // For polling SWS[1:0]（system clock status）

  // Clear the nibbles in CFGR 
  // 0xF8FF0000 to clear：SW(1:0)、HPRE(7:4)、PPRE1(10:8)、PPRE2(13:11)、
  // ADCPRE(15:14)、PLLSRC(16)、PLLXTPRE(17)、PLLMUL(21:18)、USBPRE(22)、MCO(26:24)
  RCC->CFGR &= 0xF8FF0000;

  // Turn off source clocks
  // 0xFEF6FFFF clears：PLLON(bit24)、CSSON(bit19)、HSEBYP(bit18)、HSEON(bit16)
  RCC->CR   &= 0xFEF6FFFF;

  // Turn on external crystal resonator HSE（8 MHz）
  RCC->CR   |= 0x00010000;     // HSEON=1 (bit16)

  // wait for HSE to be ready（HSERDY=1，bit17）。，can also use (RCC->CR & (1u<<17)) 
  while (!(RCC->CR >> 17));

  // Set APB1 frequency division as /2（PCLK1 MAXES out at 36 MHz）
  RCC->CFGR  = 0x00000400;     // PPRE1[10:8]=100b → /2

  //  PLL ：×9（PLLMUL[21:18]）
  RCC->CFGR |= PLL << 18;

  // Set PLL Source as HSE（PLLSRC=1）
  RCC->CFGR |= 1 << 16;
  RCC->CFGR |= 1 << 16;       

  // Flash access ：2 wait cycles + Prefetch Buffer Enable（Needed for 72 MHz）
  // 0x32 ：LATENCY[2:0]=010b(2WS) + PRFTBE(bit4)=1
  FLASH->ACR |= 0x32;
  ...
  // Clock source PLL（SW=10）
  RCC->CFGR |= 0x00000002;

  // Comfirm：SWS[3:2] if reads 10, indicates SYSCLK is from PLL
  while (temp != 0x02) {
    temp  = RCC->CFGR >> 2;    // Shift SWS[3:2] to lowest 2 bits
    temp &= 0x03;              // take the lowest 2 bits
  }

  // 至此：HCLK=72 MHz，PCLK1=36 MHz，PCLK2=72 MHz
}
...
int main(void)
{
  ...
  const char msg1[] = "Exp2.3 delay-based Usart 2: ABCDEFG\r\n";
  const char msg2[] = "Exp2.3 TXE-based Usart 2: 0123456789 abcdefghijklmnopqrstuvwxyz\r\n";
	const char msg3[] = "Exp2.3 TXE-based Usart 1: 0123456789 abcdefghijklmnopqrstuvwxyz\r\n";
  ...
  Delay(1000000);                              // BUFFER TIME FOR USART1

  // FIRST USE delay 
  usart_print(1, (char*)id);
  usart_print(2, (char*)msg1);

  // THEN USE TXE
  usart_print_txe(2, (char*)msg2);
	usart_print_txe(1, (char*)msg3);
